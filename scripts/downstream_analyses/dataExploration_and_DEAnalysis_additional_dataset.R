...
#==========================
read_counts_df <- read_tsv(path_read_count) %>% column_to_rownames(var = 'gene_id')

metadata_df <- read_tsv(path_metadata) %>% column_to_rownames(var = 'SampleCode')

# Remove the outliers
sample_to_take <- metadata_df %>% rownames_to_column(var = 'sample') %>% 
  filter(Source %in% c('lung') & !sample %in% c('case4-lung1', 'case4-lung2', 'case5-lung2')) %>%
  pull(sample)
...
for (case in  metadata_sub_df %>% filter(!Case %in% c('control')) %>% pull(Case) %>% unique()){
  ...
  dir_output = file.path(opt$o, case)
  
  sub_sample_to_take <- metadata_sub_df %>% rownames_to_column(var = 'sample') %>% filter(Case %in% c(case, 'control')) %>% pull(sample)
  
  metadata_sub_sub_df <- metadata_df %>% rownames_to_column(var = 'sample') %>% filter(sample %in% sub_sample_to_take) %>% column_to_rownames(var = 'sample')
  ...
  diff_expr_analysis(
    read_counts_df = read_counts_df %>% select(all_of(sub_sample_to_take)),
  ...
  DESeq.ds <- DESeqDataSetFromMatrix(
    countData = read_counts_df %>% select(all_of(sub_sample_to_take)),
