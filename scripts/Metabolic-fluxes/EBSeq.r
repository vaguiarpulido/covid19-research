...
run_ebseq(count_table,factor(DE, levels=c("SARS-CoV-2", "Mock")),outfile)
...
run_ebseq(count_table,factor(DE, levels=c("RSV", "Mock")),outfile)

### Series4
count_table<-countData[,c(19,20,17,18)]
DE=as.factor(c("Mock","Mock","IAV","IAV"))
outfile=paste("Series4.csv",sep="")
run_ebseq(count_table,factor(DE, levels=c("IAV", "Mock")),outfile)
...
run_ebseq(count_table,factor(DE, levels=c("HPIV3", "Mock")),outfile)
